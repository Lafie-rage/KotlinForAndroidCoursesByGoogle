...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        when (lemonadeState) {
            SELECT -> {
                ...
                Log.d("LemonSize", lemonSize.toString())
                squeezeCount = 0
            }
            SQUEEZE -> {
                squeezeCount++
    ...
    private fun setViewElements() {
        ...
        lemonImage?.let { image ->
            when (lemonadeState) {
                SELECT -> {
                    textAction.text = getString(R.string.lemon_select)
                    image.setImageDrawable(AppCompatResources.getDrawable(this, R.drawable.lemon_tree))
                }
                SQUEEZE -> {
                    textAction.text = getString(R.string.lemon_squeeze)
                    image.setImageDrawable(AppCompatResources.getDrawable(this, R.drawable.lemon_squeeze))
                }
                DRINK -> {
                    textAction.text = getString(R.string.lemon_drink)
                    image.setImageDrawable(AppCompatResources.getDrawable(this, R.drawable.lemon_drink))
                }
                else -> { // RESTART STATE
                    textAction.text = getString(R.string.lemon_empty_glass)
                    image.setImageDrawable(AppCompatResources.getDrawable(this, R.drawable.lemon_restart))
                }
            }
        }
    }
